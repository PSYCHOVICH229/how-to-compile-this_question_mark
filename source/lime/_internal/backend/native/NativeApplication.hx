			...
			if (keyCode == RETURN)
			{
				if (type == KEY_DOWN)
				{
						...
						&& (!modifier.ctrlKey && !modifier.shiftKey && !modifier.metaKey))
					{
						...
						{
							var fullscreen:Bool = !window.fullscreen;

							ClientPrefs.prefs.set('fullscreen', fullscreen);
							ClientPrefs.onChangeSetting('fullscreen');

							var fullscreenOption:Option = VisualsUISubState.fullscreenOption;
							var instance:BaseOptionsMenu = BaseOptionsMenu.instance;

							if (instance != null && fullscreenOption != null)
							{
								fullscreenOption.setValue(fullscreen);
								instance.reloadCheckboxes();
							}
						}
					}
				}
