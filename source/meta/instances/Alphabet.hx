...
class Alphabet extends FlxSpriteGroup
{
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			if (changeX)
				x = FlxMath.lerp(x, startPosition.x - Math.abs(targetY * distancePerItem.x), lerpVal);
			if (changeY)
	...
	public function snapToPosition()
	{
		...
		{
			if (changeX)
				x = startPosition.x - Math.abs(targetY * distancePerItem.x);
			if (changeY)
...
class AlphaCharacter extends FlxSprite
{
	...
	// note to self: these all start with fucking spaces lol
	private inline static final shitBeforeSuffix:String = ' ';
	public static final suffixes = {
		normal: shitBeforeSuffix + 'normal',
		bold: shitBeforeSuffix + 'bold',
		uppercase: shitBeforeSuffix + 'uppercase',
		lowercase: shitBeforeSuffix + 'lowercase'
	};
	...
	public function new(x:Float, y:Float, character:String, bold:Bool, parent:Alphabet)
	{
		...
		else
		{
			...
			{
				suffix = (lowercase == character) ? suffixes.lowercase : suffixes.uppercase;
			}
			else
			{
				suffix = suffixes.normal;
				if (curLetter != null && curLetter.offsets != null)
				{
					for (i in 0...curLetter.offsets.length)
						letterOffset[i] = curLetter.offsets[i];
				}
			}
		}

		var anim:String = ((curLetter != null && curLetter.anim != null) ? curLetter.anim : lowercase) + suffix;
	...
	public inline static function isTypeAlphabet(c:String) // thanks kade
	{
	...
	public function updateLetterOffset()
	{
		if (animation.curAnim == null)
			return;
		if (!animation.curAnim.name.endsWith(suffixes.bold))
			offset.y -= 110 - height;
	}
}
