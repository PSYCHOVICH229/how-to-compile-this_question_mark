...
{
	public inline static final NO_FILTER:Int = 0;
	public inline static final ONLY_ALPHA:Int = 1;
	public inline static final ONLY_NUMERIC:Int = 2;
	public inline static final ONLY_ALPHANUMERIC:Int = 3;
	public inline static final CUSTOM_FILTER:Int = 4;

	public inline static final ALL_CASES:Int = 0;
	public inline static final UPPER_CASE:Int = 1;
	public inline static final LOWER_CASE:Int = 2;

	public inline static final BACKSPACE_ACTION:String = "backspace"; // press backspace
	public inline static final DELETE_ACTION:String = "delete"; // press delete
	public inline static final ENTER_ACTION:String = "enter"; // press enter
	public inline static final INPUT_ACTION:String = "input"; // manually edit
	public inline static final PASTE_ACTION:String = "paste"; // text paste
	public inline static final COPY_ACTION:String = "copy"; // text copy
	public inline static final CUT_ACTION:String = "cut"; // text copy
	...
	private function set_customFilterPattern(cfp:EReg)
	{
	...
	/**
	 * Whether or not pressing enter stops focusing on the input.
	 */
	public var useReturnToUnfocus:Bool = true;
	...
	private function set_caretColor(i:Int):Int
	{
		caretColor = i;
		dirty = true;
		return caretColor;
	...
	private function set_caretWidth(i:Int):Int
	{
		caretWidth = i;
		dirty = true;
		return caretWidth;
	...
	private function mouseOverlapping()
	{
	...
	**/
	...
	{
		// Enter
		if (hasFocus && e.keyCode == 13 && useReturnToUnfocus)
		{
			var hadFocus:Bool = hasFocus;
			hasFocus = false;
	...
	private function onKeyDown(e:KeyboardEvent):Void
	{
		if (hasFocus)
		{
					...
					case 67:
						{
							...
							// Stops the function to go further, because it would type in a c to the input
							return;
						}
			...
			switch (key)
			{
				// Do nothing for Enter, Shift, Ctrl, Esc, and flixel console hotkey
				case 0 | 13 | 16 | 17 | 220 | 27:
					return;
				// Left arrow
				case 37:
					{
				...
				default:
					{
						var char:Int = e.charCode;
						if (char > 0)
						{
							// Actually add some text
							var newText:String = filter(String.fromCharCode(char));
							if (newText.length > 0 && (maxLength <= 0 || (text.length + newText.length) < maxLength))
							{
								text = insertSubstring(text, newText, caretIndex);
								caretIndex++;
	...
	private override function calcFrame(RunOnCpp:Bool = false):Void
	{
		...
		if (backgroundSprite != null)
		{
			if (background)
			{
				var previousAlpha:Float = backgroundSprite.alpha;

				backgroundSprite.makeGraphic(Std.int(width), Std.int(height), backgroundColor);
				backgroundSprite.alpha = previousAlpha;
	...
	private function set_hasFocus(newFocus:Bool):Bool
	{
		if (newFocus)
		{
			...
			{
				_caretTimer = new FlxTimer().start(.5, toggleCaret, 0);
				caret.visible = true;
				caretIndex = text.length;
			}
		}
