...
{
	public inline static final deathAmount:Int = 3;

	private inline static final prompt:String = "press ACCEPT to turn off mechanics,\notherwise press CANCEL to keep them on";
	private inline static final tooManyLetters:Int = 128;
	...
	override public function create()
	{
		...
		var songName:String = song?.song ?? 'this song';
		...
		texts = [
			...
			'listen while youre here i might as well tell you how to unlock the secret "Bend Hard" song\nokay so you gotta press ACCEPT on this screen right now and youll unlock the song, youll have to beat the current song first tho! good luck',
			...
			'if you dont disable mechanics the helicopter will come to steal your horse cheese farm. you will lose all horses and become pyromaniac so $prompt',
			'the pirates are comiong to. STEALER YOURTREASURE!! press ACCEPT to stop them!',
			'press the fucking spacebar/avoid the fucking horse cheese notes you dumbass',
			...
			'press accept to magically remove the shuttlecock from badminton',
		...
		var str:String = texts[index] ?? prompt;

		text = new FlxText(0, 0, FlxG.width * .8, str, Math.round(32 * FlxMath.bound(tooManyLetters / str.length, .5, 1)));
			...
			if (accept || back)
			{
				...
				FlxG.camera.fade(FlxColor.BLACK, 1, false, function()
				{
					FlxG.sound.music?.fadeOut(1, 0, function(twn:FlxTween)
					{
						...
						{
							if (PlayState.deathCounter < deathAmount)
