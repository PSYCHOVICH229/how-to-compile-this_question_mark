...
import meta.instances.TypedAlphabet;
...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		daText = new TypedAlphabet(DialogueBox.DEFAULT_TEXT_X, DialogueBox.DEFAULT_TEXT_Y, DEFAULT_TEXT);
	...
	private function addDialogueLineUI()
	{
		...
		speedStepper = new FlxUINumericStepper(10, characterInputText.y + 40, .005, .05, 0, .5, 3);
	...
	private function updateTextBox()
	{
		box.flipX = false;
	...
	private static var DEFAULT_SPEED:Float = .05;
	private static var DEFAULT_BUBBLETYPE:String = "normal";

	private function reloadText(skipDialogue:Bool)
	{
		...
		if (daText.rows > 2)
			daText.y -= DialogueBox.LONG_TEXT_ADD;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender == speedStepper))
		{
			...
			if (Math.isNaN(dialogueFile.dialogue[curSelected].speed)
				...
				|| dialogueFile.dialogue[curSelected].speed < (1 / 1000))
			{
				dialogueFile.dialogue[curSelected].speed = 0;
			}
	...
	private function changeText(add:Int = 0)
	{
	...
	private function characterAnimSpeed()
	{
		if (character.animation.curAnim != null)
		{
			var speed:Float = speedStepper.value;
			var rate:Float = FlxMath.bound(24 - (((speed - .05) / 5) * 480), 12, 48);
