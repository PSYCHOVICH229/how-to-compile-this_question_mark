...
{
	inline static var defaultAnim:String = 'bf';
	...
	var curAnim:Int = 0;
	var daAnim:String = defaultAnim;
	var goToPlayState:Bool = true;
	var camFollow:FlxObject;
	var lastLibrary:String = null;

	public function new(daAnim:String = defaultAnim, goToPlayState:Bool = true, ?lastLibrary:String)
	{
		super();
		this.daAnim = daAnim;
	...
	{
		// FlxG.sound.playMusic(Paths.music('breakfast'), .5);
		...
		healthBarBG = new FlxSprite(0, FlxG.height - 40).makeGraphic(400, 30, FlxColor.WHITE);
	...
	private function reloadBGs()
	{
		...
		var bg:BGSprite = new BGSprite('stageback', -600 + OFFSET_X - playerXDifference, -300);
		bgLayer.add(bg);
	...
	private function addSettingsUI()
	{
		var tab_group = new FlxUI(null, UI_box);

		tab_group.name = "Settings";
		playableCheckBox = new FlxUICheckBox(10, 60, null, null, "Playable Character", 100);

		playableCheckBox.checked = char.playable;
		playableCheckBox.callback = function()
		{
		...
		charDropDown = new FlxUIDropDownMenuCustom(10, 30, FlxUIDropDownMenuCustom.makeStrIdLabelArray([''], true), function(character:String)
		{
			...
			// playableCheckBox.checked = daAnim.startsWith('bf');
		...
		tab_group.add(playableCheckBox);
	...
	var playableCheckBox:FlxUICheckBox;
	...
	private function addCharacterUI()
	{
		...
		tab_group.camera = camMenu;
		tab_group.cameras = [camMenu];
		...
		singDurationStepper = new FlxUINumericStepper(15, healthIconInputText.y + 45, .1, 4, 0, 999, 1);
		scaleStepper = new FlxUINumericStepper(15, singDurationStepper.y + 40, .05, 1, .05, 10, 2);
		...
		healthIconInputText.camera = camMenu;
		imageInputText.camera = camMenu;

		healthIconInputText.cameras = [camMenu];
		imageInputText.cameras = [camMenu];
	...
	private function addAnimationsUI()
	{
		var tab_group = new FlxUI(null, UI_box);
		...
		animationInputText.camera = camMenu;
		animationNameInputText.camera = camMenu;
		animationIndicesInputText.camera = camMenu;
		animationNameFramerate.camera = camMenu;
		animationLoopCheckBox.camera = camMenu;

		animationInputText.cameras = [camMenu];
		animationNameInputText.cameras = [camMenu];
		animationIndicesInputText.cameras = [camMenu];
		animationNameFramerate.cameras = [camMenu];
		animationLoopCheckBox.cameras = [camMenu];
	...
	private function reloadCharacterImage()
	{
		var lastAnim:String = '';
		if (char.animation.curAnim != null)
	...
	private function loadChar(blahBlahBlah:Bool = true)
	{
		...
		ghostChar = new Character(0, 0, daAnim, blahBlahBlah, true);
		ghostChar.alpha = .6;

		char = new Character(0, 0, daAnim, blahBlahBlah, true);
	...
	private function updatePointerPos()
	{
	...
	private function findAnimationByName(name:String):AnimArray
	{
		for (anim in char.animationsArray)
		{
			if (anim.anim == name)
	...
	private function reloadCharacterOptions()
	{
	...
	private function reloadAnimationDropDown()
	{
	...
	private function reloadCharacterDropDown()
	{
	...
	private function resetHealthBarColor()
	{
	...
	override function update(elapsed:Float)
	{
		...
		if (!charDropDown.dropPanel.visible)
		{
			if (FlxG.keys.justPressed.ESCAPE)
			{
				if (goToPlayState)
				{
					Paths.setCurrentLevel(lastLibrary);
			...
			var delta:Int = CoolUtil.delta(FlxG.keys.pressed.E, FlxG.keys.pressed.Q);
			if (delta != 0)
				camEditor.zoom = FlxMath.bound(camEditor.zoom + (elapsed * camEditor.zoom * delta), .1, 3);
	...
	private function saveCharacter()
	{
		var json = {
