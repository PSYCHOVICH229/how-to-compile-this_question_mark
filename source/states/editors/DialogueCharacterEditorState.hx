...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function create()
	{
		...
		daText = new TypedAlphabet(DialogueBox.DEFAULT_TEXT_X, DialogueBox.DEFAULT_TEXT_Y, '', .05, false);
	...
	private function reloadAnimationsDropDown()
	{
		animationArray = [];
	...
	private function addCharacterUI()
	{
		...
		scaleStepper = new FlxUINumericStepper(imageInputText.x, xStepper.y + 50, .05, character.jsonFile.scale, .1, 10, 2);
	...
	private function updateCharTypeBox()
	{
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput && !animationDropDown.dropPanel.visible)
		{
			...
			if (FlxG.keys.pressed.Q && camGame.zoom > .1)
			{
